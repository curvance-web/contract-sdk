...
Deep lookup reference for `curvance-app-v2`. Consult specific `##` sections as needed via the reference table in `Skill_CurvanceApp.md`.
...
│   ├── earn/                       # Vault detail page (built — vault-detail-page, vault-sidebar, vault-header, types, constants)
...
│   ├── earn/                       # Vault module (built — mirrors marketv2 patterns, mock data)
...
`SelectedRowCard` is a single shared component used in both the desktop sidebar (L3527, via `DashboardViewManager`) and the mobile drawer (L2873, via `ManagePosition`). It uses CSS breakpoints to show different content:
...
**Values should be computed once at the top of the component**, not inline per breakpoint. The deposit display follows this correctly (L3083-3084: computes `userDepositsUsd` and `userDepositTokens` once, references in both desktop and mobile JSX). The collateral display does NOT — it calls `getUserCollateral(false)` inline in JSX and shows shares labeled as tokens (BUG-DISPLAY-007). When adding new data rows to this component, always compute at the top and reference in both paths.

The desktop expanded row (`DashboardExpandedRowContent` at L2504) is a **separate component** that matches column widths to the table header via ResizeObserver. It shows: Price, Liquidation Price, Collateral Cap, LTV, Deposit vAPY. This component does NOT share data computation with `SelectedRowCard`.
...
| `usdFormatter` / `tokenFormatter` | ✅ Replaced | Built `v2-formatters.ts` — `formatSidebarUSD`, `formatSidebarToken`, `inputFontSize`, `ghostFontSize`. Import: `@/utils/v2-formatters` |
...
## Bytes & Engagement Features

> **Migrated** to `Reference_CurvanceBytes.md`. See that file for: Bustabyte game design (state machine, multiplier tiers, edge cases), partner task system (notification panel, task detection, completion flow), achievement badge inventory (24 badges), share card specs, referral page layout, rank tier visuals.

> Onboarding tour UI patterns remain in `Reference_AerariumUI.md` → Onboarding Tour Patterns (UI implementation specs, not engagement feature logic).
...
- `getDepositApy(token, opportunities, apyOverrides?)` — total deposit APY (native + interest + Merkl)
- `getBorrowCost(token, opportunities)` — net borrow cost (rate - Merkl incentives, can be negative)
...
- Feature gate (app-side): `NEXT_PUBLIC_ENABLE_MERKL_OPPORTUNITIES === 'true'`

---

## User Onboarding Guide

Two guided tour tracks designed in Figma. Implementation deferred — timeline constraint.

**Deposit tour (15 steps):** Dashboard overview → navigation → explore page → market selection → market sidebar → deposit flow → position management → collateral → leverage → health factor → rewards.

**Borrow tour (10 steps):** Borrow-specific sidebar elements → token selection → amount input → borrow confirmation + 3-step loan detail sub-tour (debt display, repay action, interest tracking).

**Tooltip card spec:** Step counter (e.g., "3/15"), title, description, Previous/Next buttons (brand purple CTA), blue dashed border on target element highlighting the focused UI component.

**Both mobile and desktop variants** designed in Figma. Full step-by-step inventory in `Reference_AerariumUI.md` → Onboarding Tour Patterns.
