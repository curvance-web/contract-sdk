---
name: curvance-v1
description: "Use when working on or navigating the curvance-app-v2 codebase (v1 frontend). Triggers: adding pages/routes, creating modules, modifying navigation, importing v1 components, deploying to Vercel, understanding module boundaries, any file creation/editing in the repo. Compose with Skill_AerariumUI.md for UI/design rules. Do NOT use for Aerarium v2 (clean-slate frontend) or Solidity/protocol work."
---

# Curvance V1 Codebase
...
Before building new UI components, check `modules/marketv2/components/market-detail/primitives/` — DeltaBadge, LiquidityBadge, RingGauge, InfoIcon, Pills, CapacityRow, MetricRow, HoverRow, ChartTooltip. Full inventory: Reference_CurvanceV1.md → Market Detail Primitives.
...
| Using `<input type="range">` with controlled React `value` | Browser sets position natively → React reconciliation resets → deadlock after click-to-position | Replace ALL range inputs with custom `mouseDown`/`mouseMove`/`mouseUp` on a track div. Zero `<input type="range">` should remain |
...
**File:** `Reference_CurvanceV1.md` (773 lines)
...
| Market Detail Design Decisions | 673-741 | Typography scale, chart header contract, sidebar dims, stat card layout |
| Table Column Definitions | 448-503 | All 6 tables — columns, data types, responsive breakpoints |
| Dashboard Page Architecture | 536-587 | State machine, earnings calc, layout composition |
...
| Utility Functions | 607-641 | Health factor, formatting, market utils, app utils |
| Transaction Flows | 504-535 | Record shape + post-tx cleanup |
| Dead Code | 642-672 | Confirmed dead, likely dead, NOT dead despite age |
...
| Sidebar Reuse Decisions | 742-762 | What's reused vs rebuilt |
| Liquidation Calculations | 588-606 | Price calculation logic |
...
| Loading States | 763-773 | Loading/empty state specs |
